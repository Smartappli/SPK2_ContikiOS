...
void lll_reduce(double* b, int n, double lc, double uc)
{
  ...
  while (k < n){
	...
	b[i*n + k-1] = temp;
      ...
      double B_temp = B[k];
      double C_param = B[k] + mu[k*n + k-1]*mu[k*n + k-1]*B[k-1];
      double mu_temp = mu[k*n + k-1];
      //Now we continue with the algorithm, first adjust B
      mu[k*n + k-1] = mu_temp*B[k-1]/C_param;
      B[k] = B[k-1]*B[k]/C_param;
      B[k-1] = C_param;
      //All other B values stay the same
      //Next we adjust mu
      for (i = k+1; i < n; i++){
	double temp = mu[i*n + k-1];
	mu[i*n + k-1] = mu[i*n + k-1]*mu[k*n + k-1] + mu[i*n + k]*B_temp/C_param;
